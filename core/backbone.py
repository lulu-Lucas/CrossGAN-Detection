...
def darknet53(input_data, trainable):
    ...
    with tf.variable_scope('darknet'):
        ...
        route_2 = input_data#16倍下采样
        input_data = common.convolutional(input_data, filters_shape=(3, 3, 512, 1024),
                                          trainable=trainable, name='conv43', downsample=True)

        for i in range(4):
            input_data = common.residual_block(input_data, 1024, 512, 1024, trainable=trainable, name='residual%d' %(i+19))#32倍下采样
        # input_data1 = tf.nn.max_pool(input_data, [1, 2, 6, 1], [1, 1, 1, 1], padding='SAME')
        # input_data2 = tf.nn.max_pool(input_data, [1, 4, 12, 1], [1, 1, 1, 1], padding='SAME')
        # input_data3 = tf.nn.max_pool(input_data, [1, 8, 24, 1], [1, 1, 1, 1], padding='SAME')
        # input_data = tf.concat([input_data, input_data1, input_data2, input_data3], axis=-1)
