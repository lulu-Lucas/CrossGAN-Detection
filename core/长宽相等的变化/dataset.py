...
class Dataset(object):
    ...
    def __next__(self):
        ...
        with tf.device('/cpu:0'):
            ...
            self.train_input_sizey = self.train_input_sizex
            ...
            batch_image2 = np.zeros((self.batch_size, self.train_input_sizex, self.train_input_sizey, 3))
            batch_image3 = np.zeros((self.batch_size, self.train_input_sizex, self.train_input_sizey, 3))
            ...
            if self.batch_count < self.num_batchs:
                while num < self.batch_size:
                    ...
                    image1, image2,image3,bboxes = self.parse_annotation(annotation)
                    ...
                    num += 1
                self.batch_count += 1
                return batch_image1, batch_image2,batch_image3,batch_label_sbbox, batch_label_mbbox, batch_label_lbbox, \
    ...
    def random_horizontal_flip(self, image1,image2,image3, bboxes):
        ...
        return image1,image2,image3, bboxes

    def random_crop(self, image1, image2,image3,bboxes):
    ...
    def random_translate(self, image1,image2,image3,  bboxes):
    ...
    def parse_annotation(self, annotation):
        ...
        image2 = np.array(cv2.imread(image_path2))  # 数组形式
        image3 = np.array(cv2.imread(image_path3))  # 数组形式
        ...
        bboxes = np.array([list(map(lambda x: int(float(x)), box.split(','))) for box in line[3:]])
        if self.data_aug:
            image1,image2,image3, bboxes = self.random_horizontal_flip(np.copy(image1),np.copy(image2),np.copy(image3), np.copy(bboxes))
            image1,image2,image3, bboxes = self.random_crop(np.copy(image1),np.copy(image2),np.copy(image3), np.copy(bboxes))
            image1,image2,image3, bboxes = self.random_translate(np.copy(image1),np.copy(image2),np.copy(image3), np.copy(bboxes))

        #print(bboxes)
        image1,image2, image3,bboxes = utils.image_preporcess(np.copy(image1), np.copy(image2),np.copy(image3),self.train_input_sizex, self.train_input_sizey, np.copy(bboxes))
        ...
        return image1,image2,image3, np.array(updated_bb)
    ...
    def preprocess_true_boxes(self, bboxes):
        ...
        for bbox in bboxes:
            ...
            for i in range(3):
                ...
                anchors_xywh[:, 2:4] = self.anchors[i]*self.train_output_sizesx[i]

                iou_scale = self.bbox_iou(bbox_xywh_scaled[i][np.newaxis, :], anchors_xywh)
                iou.append(iou_scale)# 一次循环结束有9个iou
                ...
                if np.any(iou_mask):
                    ...
                    label[i][yind, xind, iou_mask, 5:] = smooth_onehot #类别
            ...
            if not exist_positive:
                ...
                best_detect = int(best_anchor_ind / self.anchor_per_scale)#选择小中大
                best_anchor = int(best_anchor_ind % self.anchor_per_scale)#在小中大某类中选择相应的anchor
        ...
        label_sbbox, label_mbbox, label_lbbox = label#分类好之后记录了每个bbox应该使用哪个anchor
